...
static int irq_setup(struct gf_dev *gf_dev)
{
	...
	if (status) {
		pr_err("right failed to request IRQ:%d\n", gf_dev->irq);
		return status;
	}
...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	FP_LOG_INFO("right received key event[%d], key=%d, value=%d\n", key_input, gf_key->key, gf_key->value);
...
{
#if (0)//TODO:Need major & slave LCD are powerdown event, But now LCD module no this event
	struct gf_dev *gf_dev;
	struct fb_event *evdata = data;
	unsigned int blank;
	char msg = 0;

	FP_LOG_INFO("right event = %d\n", (int)val);
	FP_LOG_INFO("right blank = %d\n", *(int *)(evdata->data));

	if (val != MSM_DRM_SWITCH_EARLY_EVENT_BLANK)
		return 0;
	gf_dev = container_of(nb, struct gf_dev, notifier);
	if (evdata && evdata->data && val == MSM_DRM_SWITCH_EARLY_EVENT_BLANK && gf_dev) {
...
#endif
{
	...
	if (minor < N_SPI_MINORS) {
		...
		dev_dbg(&gf_dev->spi->dev, "right no minor number available!\n");
		status = -ENODEV;
		mutex_unlock(&device_list_lock);
		goto error_hw;
	}
