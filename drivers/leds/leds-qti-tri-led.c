...
static int nubia_light_breath_set(struct led_classdev *led_cdev, ST_BREATH_FEATURE breath_param)
{
	...
	struct qpnp_led_dev *led = container_of(led_cdev, struct qpnp_led_dev, cdev);
	...
	if (output_pattern.num_entries!=0)
		led->pwm_dev->chip->ops->set_output_pattern(led->pwm_dev->chip, led->pwm_dev, &output_pattern);
						...
						const char *buf, size_t count)
{
	...
	if (back_output_pattern.num_entries != 0) {
		...
		led->pwm_dev->chip->ops->set_output_pattern(led->pwm_dev->chip, led->pwm_dev, &back_output_pattern);
	}
#endif
