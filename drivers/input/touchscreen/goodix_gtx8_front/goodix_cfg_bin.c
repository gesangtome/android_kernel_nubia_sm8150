...
extern int goodix_i2c_write(struct goodix_front_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len);
extern int goodix_front_i2c_read(struct goodix_front_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len);
extern struct goodix_module goodix_front_module;

int goodix_front_start_cfg_bin(struct goodix_ts_core *ts_core)
{
	struct task_struct *cfg_bin_thrd;
	/* create and run update thread */
	cfg_bin_thrd = kthread_run(goodix_front_cfg_bin_proc,
...
int goodix_front_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
	u8 checksum;
	int i, r;
	u16 offset1, offset2;
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		goodix_front_cfg_pkg_leToCpu(&cfg_bin->cfg_pkgs[i]);
...
int goodix_front_cfg_bin_proc(void *data)
{
	struct goodix_ts_core *core_data = data;
	struct goodix_front_ts_device *ts_dev = core_data->ts_dev;
	...
	r = goodix_front_read_cfg_bin(dev, cfg_bin,core_data);
	if (r < 0) {
		ts_err("read cfg_bin from /etc/firmware FAILED");
		goto exit;
	}
	/*parse cfg bin*/
	r = goodix_front_parse_cfg_bin(cfg_bin);
	if (!r) {
	...
	r = goodix_front_get_reg_and_cfg(ts_dev, cfg_bin);
	if (!r) {
		ts_info("get reg and cfg from cfg_bin SUCCESS");
	} else {
		if (r != -EBUS) {
			ts_err("get reg and cfg from cfg_bin FAILED, update fw then retry");
			goodix_front_module.core_data = core_data;
			goodix_front_module.core_exit = false;
			complete_all(&goodix_front_module.core_comp);
			goto exit;
		} else {
	...
	if (core_data->cfg_init) {
		ts_err("first cfg send already, not register new event");
		goto exit;
	}

	/* alloc/config/register input device */
	r = goodix_front_ts_input_dev_config(core_data);
	if (r < 0)
		goto exit;

	/* request irq line */
	r = goodix_front_ts_irq_setup(core_data);
	if (r < 0)
		goto exit;
	...
	goodix_front_module.core_data = core_data;
	goodix_front_module.core_exit = false;
	core_data->cfg_init = 1;
	/*complete_all(&goodix_modules.core_comp);*/

#ifdef CONFIG_FB
	core_data->fb_notifier.notifier_call = goodix_front_ts_fb_notifier_callback;
	...
	/* esd protector */
	goodix_front_ts_esd_init(core_data);

	/* generic notifier callback */
	core_data->ts_notifier.notifier_call = goodix_front_generic_noti_callback;
	goodix_front_ts_register_notifier(&core_data->ts_notifier);
...
int goodix_front_get_reg_and_cfg(struct goodix_front_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin)
{
	int i;
	u16 addr;
	u8 read_len;
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		r = goodix_front_i2c_read(ts_dev, addr,
				&temp_sensor_id, 1);
		...
		if (!addr || !cfg_bin->cfg_pkgs[i].cnst_info.fw_mask[0]) {
			...
			r = goodix_front_i2c_read(ts_dev, addr, temp_fw_mask, sizeof(temp_fw_mask));
			if (r < 0) {
		...
		r = goodix_front_i2c_read(ts_dev, addr, temp_pid, read_len);
		if (r < 0) {
...
int goodix_front_read_cfg_bin(struct device *dev, struct goodix_cfg_bin *cfg_bin,
							struct goodix_ts_core *core_data)
{
	...
	/*get cfg_bin_diff_name*/
	switch (core_data->diff_cfg_mode) {
		case GOODIX_CFG_V0:
			strlcpy(cfg_bin_name, TS_DEFAULT_CFG_BIN_V0,
				sizeof(cfg_bin_name));
			break;
		case GOODIX_CFG_V1:
			strlcpy(cfg_bin_name, TS_CFG_BIN_V1,
				sizeof(cfg_bin_name));
			break;
		case GOODIX_CFG_V2:
			strlcpy(cfg_bin_name, TS_CFG_BIN_V2,
				sizeof(cfg_bin_name));
			break;
		default:
			break;
...
int goodix_front_read_cfg_bin_from_dts(struct device_node *node, struct goodix_cfg_bin *cfg_bin)
{
	unsigned int len = 0;
...
void goodix_front_cfg_pkg_leToCpu(struct goodix_cfg_package *pkg)
{
	if (!pkg) {
