...
* (at your option) any later version.sensor_common.h
*
...
#define ALS_DEV_NAME        "light_front"
...
static long cofficient_judge = 554;
static long cofficient_red[2]={340, -138};
static long cofficient_green[2] = {1586, 3490};
...
* RETURN     : Lux value
*****************************************************************************/
static int rgb_bh1745_calc_lx(struct i2c_client *client, struct rgb_bh1745_data *rdata, int mode_ctrl2, unsigned short itime)
{
	long long int lx;
	...
	lx= lx_tmp*16/gain/(itime/160)/1000;
	...
	{
		mode_ctrl2 &= ~0x03;
		mode_ctrl2 |= ADC_GAIN_X2;
		ret = rgb_bh1745_i2c_write(client, BH1745_MODECONTROL2, mode_ctrl2, BH1745_I2C_BYTE);
		if (ret < 0) {
			SENSOR_LOG_ERROR("i2c error,mode_ctrl2 = 0x%x\n", mode_ctrl2);
		}
		SENSOR_LOG_DEBUG("modify rgbc gain = 0x%x\n", bh1745_again[mode_ctrl2&0x03]);
	}
	else if(gain == 2 && lx < 4500)
	{
		mode_ctrl2 &= ~0x03;
		mode_ctrl2 |= ADC_GAIN_X16;
		ret = rgb_bh1745_i2c_write(client, BH1745_MODECONTROL2, mode_ctrl2, BH1745_I2C_BYTE);
		if (ret < 0) {
			SENSOR_LOG_ERROR("i2c error,mode_ctrl2 = 0x%x\n", mode_ctrl2);
		}
...
static int rgb_bh1745_als_rgbc_sample(struct rgb_bh1745_data *data, struct rgbc_parameter *rgbc)
{
	...
	if (ret &= MODECONTROL2_VALID) {
		...
		SENSOR_LOG_DEBUG_IF(data->debug_level,"the data is not update\n");
		return 1;
	}
	return 0;
...
static int rgb_bh1745_get_lux(struct rgb_bh1745_data *data, bool report_event)
{
		...
		|| (data->rgb_data.clear < 0))
	{
		...
		luxValue = rgb_bh1745_calc_lx(client, data, tmp, time);
	}
	if (luxValue >= 0) {
		ret = 0;
...
static int rgb_bh1745_input_init(struct rgb_bh1745_data *data)
{
	...
	data->input_dev_als = virtual_als_input_dev;
#if 0
	...
	data->input_dev_als->name = ALS_DEV_NAME;
...
static struct device_attribute attrs_rgb_device[] = {
	...
	__ATTR(module_tpcolor ,0664, read_tp_parameters, write_module_tpcolor),
	__ATTR(tp_cfg, 0664, attr_rgb_config_tpinfo_show, attr_rgb_config_tpinfo_store),
	__ATTR(fac_calibrate,0664, attr_rgb_factory_cal_show, attr_rgb_factory_cal_store),
	__ATTR(dev_init, 0664, attr_rgb_dev_init_show, attr_rgb_dev_init_store),
				...
				const struct i2c_device_id *id)
{
	...
	SENSOR_LOG_INFO("bh1745 probe start.\n");

	data = kzalloc(sizeof(struct rgb_bh1745_data), GFP_KERNEL);
	if (IS_ERR_OR_NULL(data)) {
		SENSOR_LOG_ERROR("bh1745 Failed to allocate memory\n");
		err = -ENOMEM;
		goto exit;
	}
	...
	if (client->dev.of_node) {
		...
		if (err) {
			SENSOR_LOG_ERROR("bh1745 sensor_parse_dt() err\n");
			goto exit;
		}
	} else {
		pdata = client->dev.platform_data;
		if (!pdata) {
			SENSOR_LOG_ERROR("bh1745 No platform data\n");
			err = -ENODEV;
			goto exit;
		}
	}
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE)) {
		SENSOR_LOG_ERROR("bh1745 Failed to i2c_check_functionality\n");
		err = -EIO;
		goto exit;
	}
	...
	/*init rgb class */
	rgb_class = class_create(THIS_MODULE, ALS_DEV_NAME);
	data->rgb_dev = device_create(rgb_class, NULL, bh1745_rgb_dev_t, &rgb_bh1745_driver ,ALS_DEV_NAME);
	...
	err = sensor_create_sysfs_interfaces(data->rgb_dev, attrs_rgb_device, ARRAY_SIZE(attrs_rgb_device));
	if (err < 0) {
...
create_sysfs_interface_error:
	sensor_remove_sysfs_interfaces(data->rgb_dev, attrs_rgb_device, ARRAY_SIZE(attrs_rgb_device));
