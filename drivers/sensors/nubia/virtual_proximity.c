...
#define TMD2725_PS_SYS_PATH "/sys/class/proximity_back/proximity_back/"
#define TMD2725_PS_SYS_ENABLE_PATH "/sys/class/proximity_back/proximity_back/enable"
#define TMD2725_PS_SYS_INIT_PATH "/sys/class/proximity_back/proximity_back/prox_init"

#define TMD2725_PS_SYS_prox_offset "/sys/class/proximity_back/proximity_back/prox_offset"
#define TMD2725_PS_SYS_debug "/sys/class/proximity_back/proximity_back/debug"
#define TMD2725_PS_SYS_FLUSH_PATH "/sys/class/proximity_back/proximity_back/prox_value"
#define TMD2725_PS_SYS_prox_offset_cal  "/sys/class/proximity_back/proximity_back/prox_offset_cal"
#define TMD2725_PS_SYS_prox_thres_min "/sys/class/proximity_back/proximity_back/prox_thres_min"
#define TMD2725_PS_SYS_prox_thres_max "/sys/class/proximity_back/proximity_back/prox_thres_max"
#define TMD2725_PS_SYS_prox_thres "/sys/class/proximity_back/proximity_back/prox_thres"
#define TMD2725_PS_SYS_prox_debug "/sys/class/proximity_back/proximity_back/prox_debug"
#define TMD2725_PS_SYS_prox_uncover_min  "/sys/class/proximity_back/proximity_back/prox_uncover_min"
#define TMD2725_PS_SYS_prox_uncover_max  "/sys/class/proximity_back/proximity_back/prox_uncover_max"
#define TMD2725_PS_SYS_prox_first_event  "/sys/class/proximity_back/proximity_back/prox_first_event"
#define TMD2725_PS_SYS_prox_persit  "/sys/class/proximity_back/proximity_back/prox_persist"


#define STK3332_PS_SYS_PATH "/sys/class/proximity_front/proximity_front/"
#define STK3332_PS_SYS_ENABLE_PATH "/sys/class/proximity_front/proximity_front/enable"
#define STK3332_PS_SYS_INIT_PATH "/sys/class/proximity_front/proximity_front/prox_init"
#define STK3332_PS_SYS_FLUSH_PATH "/sys/class/proximity_front/proximity_front/prox_value"

#define STK3332_PS_SYS_prox_offset  "/sys/class/proximity_front/proximity_front/prox_offset"
#define STK3332_PS_SYS_debug  "/sys/class/proximity_front/proximity_front/debug"
#define STK3332_PS_SYS_prox_offset_cal  "/sys/class/proximity_front/proximity_front/prox_offset_cal"
#define STK3332_PS_SYS_prox_thres_min  "/sys/class/proximity_front/proximity_front/prox_thres_min"
#define STK3332_PS_SYS_prox_thres_max  "/sys/class/proximity_front/proximity_front/prox_thres_max"
#define STK3332_PS_SYS_prox_thres  "/sys/class/proximity_front/proximity_front/prox_thres"
#define STK3332_PS_SYS_prox_debug  "/sys/class/proximity_front/proximity_front/prox_debug"
#define STK3332_PS_SYS_prox_uncover_min  "/sys/class/proximity_front/proximity_front/prox_uncover_min"
#define STK3332_PS_SYS_prox_uncover_max  "/sys/class/proximity_front/proximity_front/prox_uncover_max"
#define STK3332_PS_SYS_prox_first_event  "/sys/class/proximity_front/proximity_front/prox_first_event"
#define STK3332_PS_SYS_prox_persit  "/sys/class/proximity_front/proximity_front/prox_persist"
...
static void virtual_proximity_enable(int fb_type)
{
    ...
    {
        case PROX_FRONT:
            sensor_write_file(TMD2725_PS_SYS_ENABLE_PATH, disable_buf, sizeof(disable_buf));
            sensor_write_file(STK3332_PS_SYS_ENABLE_PATH, enable_buf, sizeof(enable_buf));
            break;

        case PROX_BACK:
            sensor_write_file(STK3332_PS_SYS_ENABLE_PATH, disable_buf, sizeof(disable_buf));
            sensor_write_file(TMD2725_PS_SYS_ENABLE_PATH, enable_buf, sizeof(enable_buf));
            break;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            ...
            ret = sensor_read_file(STK3332_PS_SYS_ENABLE_PATH, &enable,1);
            SENSOR_LOG_INFO("PROX_FRONT read enable is %c\n",enable);
            break;

        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_ENABLE_PATH, &enable, 1);
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_thres_min, rbuf,sizeof(rbuf));
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_thres_min, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_min);
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_thres_max, rbuf,sizeof(rbuf));
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_thres_max, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
                ...
                const char *buf, size_t size)
{
    ...
    {
        case PROX_FRONT:
            ret=sensor_write_file(STK3332_PS_SYS_prox_thres, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",val);
            break;

        case PROX_BACK:
            ret=sensor_write_file(TMD2725_PS_SYS_prox_thres, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",val);
            break;
        default:
            return -EINVAL;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    {
        case PROX_FRONT:
            ret = sensor_read_file(STK3332_PS_SYS_prox_debug, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;

        case PROX_BACK:
            ret = sensor_read_file(TMD2725_PS_SYS_prox_debug, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
                ...
                const char *buf, size_t size)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            //ret = sensor_read_file(STK3332_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(STK3332_PS_SYS_prox_debug, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",val);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
           // ret = sensor_read_file(TMD2725_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(TMD2725_PS_SYS_prox_debug, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",val);
            break;
        default:
            return -EINVAL;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_uncover_min, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_uncover_min, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_uncover_max, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_uncover_max, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_first_event, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_first_event, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
                ...
                const char *buf, size_t size)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            //ret = sensor_read_file(STK3332_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(STK3332_PS_SYS_prox_first_event, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",val);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
           // ret = sensor_read_file(TMD2725_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(TMD2725_PS_SYS_prox_first_event, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",val);
            break;
        default:
            return -EINVAL;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_offset_cal, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_offset_cal, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
                ...
                const char *buf, size_t size)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            //ret = sensor_read_file(STK3332_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(STK3332_PS_SYS_prox_offset_cal, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",val);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
           // ret = sensor_read_file(TMD2725_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(TMD2725_PS_SYS_prox_offset_cal, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",val);
            break;
        default:
            return -EINVAL;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_debug, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_debug, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
...
/* END:   Added by xuxiaohua, 2018/9/3 */
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            //ret = sensor_read_file(STK3332_PS_SYS_prox_thres, rbuf,sizeof(rbuf));
            ret=sensor_write_file(STK3332_PS_SYS_debug, init_buf, sizeof(init_buf));
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",val);
            break;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    switch(data->fb_status)
    {
        case PROX_FRONT:
            // read  PROX_FRONT enable status
            ret = sensor_read_file(STK3332_PS_SYS_prox_offset, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_FRONT read enable is %d\n",thre_max);
            break;
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_offset, rbuf,sizeof(rbuf));
            sscanf(rbuf, "%d", &thre_max);
            SENSOR_LOG_IF(data->debug_level,"PROX_BACK read enable is %d\n",thre_max);
            break;
        default:
                ...
                const char *buf, size_t size)
{
	...
	ret = kstrtoint(buf, 0, &val);
	if(val != 0){
		SENSOR_LOG_INFO("Double prox dev init\n");
		ret=sensor_write_file(STK3332_PS_SYS_INIT_PATH, init_buf, sizeof(init_buf));
		ret=sensor_write_file(TMD2725_PS_SYS_INIT_PATH, init_buf, sizeof(init_buf));
		if (ret < 0) {
			SENSOR_LOG_ERROR("prox dev init fail\n");
			return ret;
		}
	}
    return size;
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    {
        case PROX_FRONT:
            ret = sensor_read_file(STK3332_PS_SYS_FLUSH_PATH, show_buf, sizeof(show_buf));
            break;

        case PROX_BACK:
            ret = sensor_read_file(TMD2725_PS_SYS_FLUSH_PATH, show_buf, sizeof(show_buf));;
            break;
        default:
            return -EINVAL;
    }
...
static ssize_t virtual_prox_persist_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    int ret;
	char rbuf[BUF_MAX_SIZE] = {0};
	struct virtual_proximity_data *data = dev_get_drvdata(dev);
    ...
    switch(data->fb_status)
    {
        ...
        case PROX_BACK:
            // read  PROX_BACK enable status
            ret = sensor_read_file(TMD2725_PS_SYS_prox_persit, rbuf,sizeof(rbuf));
			if(ret > BUF_MAX_SIZE)
				ret = BUF_MAX_SIZE;
			memcpy(buf, rbuf, ret);
            break;
...
static ssize_t virtual_prox_persist_store(struct device *dev,
                struct device_attribute *attr,
                const char *buf, size_t size)
{
	int ret;
	...
	switch(data->fb_status)
	{
		...
		case PROX_BACK:
			ret = sensor_write_file(TMD2725_PS_SYS_prox_persit, buf, size);
			break;
		default:
			return -EINVAL;
	...
	if (ret < 0) {
		SENSOR_LOG_ERROR("write file fail, fb = %d\n", data->fb_status);
		return ret;
	}
	return size;
}
...
static struct device_attribute attrs_prox_device[] = {
    ...
    __ATTR(prox_persist, 0664, virtual_prox_persist_show, virtual_prox_persist_store),
	__ATTR(prox_thres_to_persist, 0220, NULL, virtual_prox_thres_to_persist_store),
};

extern get_nubia_mode(void);
#define SCREEN_FRONT                    1000
#define SCREEN_BACK                     1001
#define SCREEN_ALL                      1002
#define SCREEN_SWITCHING_TOFRONT        2000
#define SCREEN_SWITCHING_TOBACK         2001
#define SCREEN_DOUBLE_SWITCHING         2005

#ifdef NUBIA_PROX_NOTIFY_LCD
                         ...
                         unsigned long event, void *data)
{
    ...
    {
		if(event == MSM_DRM_SWITCH_EARLY_EVENT_BLANK){
			blank = fb_event->data;
			panel_mode = get_nubia_mode();
			switch (panel_mode){
				case SCREEN_FRONT:
				case SCREEN_SWITCHING_TOFRONT:
				case SCREEN_ALL:
				case SCREEN_DOUBLE_SWITCHING:
					/*
					 * don`t modify to set_sensor_fb() set proximity fornt or back
					 * because android.hardware.sensors@1.0-impl.so need notify proximity_fb node
					 */
					SENSOR_LOG_INFO("set proximity is front(SCREEN Stat %d)", panel_mode);
					sensor_write_file(SET_PROXIMITY_FB_PATH, front_buf, sizeof(front_buf));
					break;
				case SCREEN_BACK:
				case SCREEN_SWITCHING_TOBACK:
					/*
					 * don`t modify to set_sensor_fb() set proximity fornt or back
					 * because android.hardware.sensors@1.0-impl.so need notify proximity_fb node
					 */
					SENSOR_LOG_INFO("set proximity is back(SCREEN Stat %d)", panel_mode);
					sensor_write_file(SET_PROXIMITY_FB_PATH, back_buf, sizeof(back_buf));
					break;
				default:
					SENSOR_LOG_ERROR("No have state to process(SCREEN Stat %d)\n", panel_mode);
				}
			}
    }
    return NOTIFY_DONE;
...
static void prox_fb_notifier_register(void){
    ...
    ret = msm_drm_register_client(&vp_data->ps_fb_notify);
    if (ret < 0){
...
int virtual_proximity_register(void)
{
    int err = 0;
...
#ifdef NUBIA_PROX_NOTIFY_LCD
	vp_data->lcd_stat = LCD_FRONT_BIT;
#endif
    vp_data->report_fb_status = 0;
    vp_data->report_value = 0;
    vp_data->enable_status = false;

    virtual_proximity_class = class_create(THIS_MODULE, VIR_PROX_DRV_NAME);
	...
	err = sensor_create_sysfs_interfaces(vp_data->proximity_dev, attrs_prox_device, ARRAY_SIZE(attrs_prox_device));
	if (err < 0) {
...
create_sysfs_interface_failed:
	sensor_remove_sysfs_interfaces(vp_data->proximity_dev, attrs_prox_device, ARRAY_SIZE(attrs_prox_device));
...
void virtual_proximity_unregister(void)
{
	...
	sensor_remove_sysfs_interfaces(dev, attrs_prox_device, ARRAY_SIZE(attrs_prox_device));
