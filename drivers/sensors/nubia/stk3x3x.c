...
#define PS_UNCOVER_DATA_MAX             8000
#define PS_THRESH_DATA_MIN              60
#define PS_THRESH_DATA_MAX              60000
...
#define PS_NAME "proximity_front"
...
static void stk3x3x_dump_reg(struct stk3x3x_data *ps_data)
{
	int ret;
	unsigned char reg[13]={0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x4F, 0xA0, 0xA1, 0xDB, 0xA4, 0x3E, 0x3F};
	uint8_t cnt;
	for(cnt = 0; cnt < 13; cnt++){
		ret = stk3x3x_i2c_smbus_read_byte_data(ps_data->client, reg[cnt]);
		if(ret < 0){
			SENSOR_LOG_ERROR("fail, ret=%d\n", ret);
		}
...
static uint32_t stk3x3x_get_ps_thd_h(struct stk3x3x_data *ps_data)
{
	unsigned char value[2];
	int err;
	err = stk3x3x_i2c_read_data(ps_data->client, STK_THDH1_PS_REG, 2, &value[0]);
	if(err < 0)
	{
		SENSOR_LOG_ERROR("fail, ret=%d\n", err);
		return err;
...
static uint32_t stk3x3x_get_ps_thd_l(struct stk3x3x_data *ps_data)
{
	unsigned char value[2];
	int err;
	err = stk3x3x_i2c_read_data(ps_data->client, STK_THDL1_PS_REG, 2, &value[0]);
	if(err < 0)
	{
		SENSOR_LOG_ERROR("fail, ret=%d\n", err);
		return err;
...
static int stk_prox_increment_init(struct stk3x3x_data *ps_data)
{
	int rc = 0;
	uint16_t data[2] = {0};
	uint32_t high_thd;
	...
	if(ps_data->cali_near_increment > PS_THRESH_INCREMENT_MAX)	{
		SENSOR_LOG_ERROR("read file value exception [%d]\n", ps_data->cali_near_increment);
		return -1;
	}

	high_thd = stk3x3x_get_ps_thd_h(ps_data);
	if (high_thd < 0)
	{
		SENSOR_LOG_DEBUG("fail, error: %d\n", high_thd);
		return high_thd;
	}
	ct_value = high_thd - ps_data->stk_ht_n_ct;
	SENSOR_LOG_ERROR("old high = %d, delta ht=%d, lt=%d\n",
				high_thd, ps_data->stk_ht_n_ct, ps_data->stk_lt_n_ct);

	SENSOR_LOG_ERROR("old smudge delta ht = %d, lt=%d\n",
				ps_data->stk_h_ht, ps_data->stk_h_lt);
	...
	if(ps_data->stk_ht_n_ct > ps_data->stk_h_ht){
		ps_data->stk_h_ht = ps_data->stk_ht_n_ct;
		ps_data->stk_h_lt = ps_data->stk_lt_n_ct;
		SENSOR_LOG_ERROR("new smudge delta ht = %d, lt=%d\n", ps_data->stk_h_ht, ps_data->stk_h_lt);
	}

    ps_data->ps_thd_h = ct_value + ps_data->stk_ht_n_ct;
    ps_data->ps_thd_l = ct_value + ps_data->stk_lt_n_ct;

	ps_data->ps_high_thd_boot = ct_value + ps_data->stk_h_ht;
	ps_data->ps_low_thd_boot = ct_value + ps_data->stk_h_lt;

	SENSOR_LOG_ERROR("thd(%d, %d), smudge thd(%d, %d)\n",
				ps_data->ps_thd_h, ps_data->ps_thd_l, ps_data->ps_high_thd_boot, ps_data->ps_low_thd_boot);
...
static int32_t stk3x3x_enable_ps(struct stk3x3x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
    ...
    if(enable)
	{
		...
		{
			ret = stk_prox_increment_init(ps_data);
			if(ret < 0)
			{
		...
		if(ps_data->boot_cali == 1 && ps_data->ps_low_thd_boot < 8000 )
		{
		...
		SENSOR_LOG_ERROR("cali crosstalk=%d, increment=%d\n", ps_data->cali_crosstalk, ps_data->cali_near_increment);
		SENSOR_LOG_ERROR("set ht=%d, lt=%d\n", ps_data->ps_thd_h, ps_data->ps_thd_l);

		if((ps_data->stk_ht_n_ct < STK_H_HT) && (ps_data->stk_h_ht > STK_H_HT)){
			ps_data->stk_h_ht = STK_H_HT;
			ps_data->stk_h_lt = STK_H_LT;
...
static int32_t stk3x3x_ps_calibration_thres(struct stk3x3x_data *ps_data){
    ...
    diff = ps_data->cali_near - ps_data->cali_crosstalk;
	SENSOR_LOG_ERROR("crosstalk = %d, near = %d, increment = %d\n", ps_data->cali_crosstalk, ps_data->cali_near, diff);
	...
	ps_data->cali_near_increment = diff;
    old_ct = ps_data->cali_crosstalk;
#if 0
    ...
    pr_info("%s: HT=%d, LT=%d\n", ps_data->ps_thd_h, ps_data->ps_thd_l);
#endif
	ps_data->ps_high_thd_boot = old_ct + ps_data->stk_h_ht;
	ps_data->ps_low_thd_boot = old_ct + ps_data->stk_h_lt;
...
static ssize_t stk_ps_distance_flush_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	input_report_rel(ps_data->ps_input_dev, REL_RZ, dist? 10:3);
	input_sync(ps_data->ps_input_dev);
...
static int stk_ps_val(struct stk3x3x_data *ps_data)
{
	...
	for(i = 0; i < 4; i++){
		ret = stk3x3x_i2c_read_data(ps_data->client, (0x24 + i*2), 2, &buf[0]);
		if(ret < 0)
		{
			SENSOR_LOG_ERROR("fail, ret=0x%x", ret);
			return ret;
...
#ifdef CALI_PS_EVERY_TIME
	ps_data->ps_high_thd_boot = ps_data->ps_stat_data[1] + ps_data->stk_h_ht;
	ps_data->ps_low_thd_boot = ps_data->ps_stat_data[1] + ps_data->stk_h_lt;
	ps_data->ps_thd_h = ps_data->ps_high_thd_boot ;
	ps_data->ps_thd_l = ps_data->ps_low_thd_boot ;
#else
...
static int stk_ps_tune_zero_init(struct stk3x3x_data *ps_data)
{
	...
	{
		ps_data->ps_high_thd_boot = STK_DEFAULT_HT;
		ps_data->ps_low_thd_boot = STK_DEFAULT_LT;
	}
#endif
	...
	hrtimer_start(&ps_data->ps_tune0_timer, ps_data->ps_tune0_delay*2, HRTIMER_MODE_REL);
...
static int stk_ps_tune_zero_func_fae(struct stk3x3x_data *ps_data)
{
	...
	if(ps_data->data_count > 10000){
	...
	}else if((ps_data->data_count%20)==6){
		flag = stk3x3x_get_flag(ps_data);
		if(flag < 0){
			return flag;
		}
		...
		SENSOR_LOG_INFO("cur ps=%d, flag=0x%x, dis=%d\n", word_data, flag, ps_data->ps_distance_last);
		SENSOR_LOG_INFO("psi_set=%d, psi=%d, smudge=%d\n", ps_data->psi_set, ps_data->psi, ps_data->ps_thd_update);
	}else if((ps_data->data_count%200)==9){
		high_thd = stk3x3x_get_ps_thd_h(ps_data);
		if (high_thd < 0)
		{
			SENSOR_LOG_DEBUG("fail, error: %d\n", high_thd);
		...
		low_thd = stk3x3x_get_ps_thd_l(ps_data);
		if (low_thd < 0)
		{
			SENSOR_LOG_DEBUG("fail, error: %d\n", low_thd);
			return low_thd;
	...
	}else if((ps_data->data_count%500)==8){
		stk3x3x_dump_reg(ps_data);
	}
	...
	if ((ps_data->psi_set != 0))
	{
		if (ps_data->ps_distance_last == 1)
		{
						...
						if(ps_data->tracking_count > 3){
							...
							if ((ps_data->ps_thd_h + STK_H_LT - STK_HT_N_CT) < ps_data->ps_high_thd_boot)
							{
								ps_data->ps_high_thd_boot =
									word_data + ps_data->stk_h_ht;
								ps_data->ps_low_thd_boot =
									word_data + ps_data->stk_h_lt;
								ps_data->boot_cali = 1;
			...
			if (ret == 0)
			{
				...
				{
					ps_data->ps_thd_h = ps_data->psi + ps_data->stk_h_ht;
					ps_data->ps_thd_l = ps_data->psi + ps_data->stk_h_lt;
					stk3x3x_set_ps_thd_h(ps_data, ps_data->ps_thd_h);
					stk3x3x_set_ps_thd_l(ps_data, ps_data->ps_thd_l);
					ps_data->ps_thd_update = true;
					SENSOR_LOG_INFO("smudge set HT=%d, LT=%d, psi=%d\n", ps_data->ps_thd_h, ps_data->ps_thd_l, ps_data->psi);
				}
			}
		}
	...
	#ifdef CALI_PS_EVERY_TIME
		if(((ps_data->psi + ps_data->stk_ht_n_ct) > (ps_data->ps_thd_h + 2000)) && (ps_data->ps_thd_h != 0))
		{
...
/*
		...
		if ((ps_data->ps_thd_h + STK_H_HT - STK_HT_N_CT) < ps_data->ps_high_thd_boot)
		{
			ps_data->ps_high_thd_boot =
				ps_data->psi + ps_data->stk_h_ht;
			ps_data->ps_low_thd_boot =
				ps_data->psi + ps_data->stk_h_lt;
			ps_data->boot_cali = 1;
...
static int stk3x3x_set_input_devices(struct stk3x3x_data *ps_data)
{
	...
	ps_data->ps_input_dev = virtual_ps_input_dev;
